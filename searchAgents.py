...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        """
        successors = []
        currentPosition, corners = state[0], state[1]
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            x, y = state[0]
...
def cornersHeuristic(state, problem):
    ...
    for corner in unvisitedCorners:
        heuristicvalue.append(manhattanDistance(coord,corner))
    result = max(heuristicvalue)
    return result
...
def foodHeuristic(state, problem):
    ...
    from util import manhattanDistance
    heuristic = [0]
    for coord in foodposition:
        heuristic.append( manhattanDistance(position, coord) )
    result = max(heuristic)
    return result
